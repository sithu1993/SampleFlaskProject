...
class User(db.Model, UserMixin):
    ...
    def can_purchase(self, purchased_item):
        return self.budget >= purchased_item.price ;
...
class Item(db.Model):
    ...
    user_owned = db.Column(db.Integer(),db.ForeignKey('user.id'))

    def __repr__(self):
        return f'Item {self.name}'
    
    def buy(self, user):
        self.user_owned=user.id
        user.budget -= self.price
        db.session.commit()

    def sell(self,user):
        self.user_owned=None
        user.budget += self.price
        db.session.commit()
